  ...
  function runCalc() {
    ...
    if (r.manual) {
      ...
      out.innerHTML =
        ...
        '<tr><td>Утильсбор</td><td>' + (r.preferential ? fmt(r.util) : '<b class="bad">коммерческий</b>') + '</td></tr>' +
        '<tr class="sum"><td>Таможня итого</td><td>' + fmt(r.total) + (r.preferential ? '' : ' + утильсбор') + '</td></tr>' +
        '</table>' +
        (r.preferential
          ? '<p class="ok">Машина проходит по льготе: до ' + RATES.util.maxHp + ' л.с. и до 3 л. Утильсбор для себя — ' + fmt(r.util) + '.</p>'
          : '<p class="bad-box">Мощность больше ' + RATES.util.maxHp + ' л.с. или объём больше 3 л — льготный утильсбор не действует. По коммерческой ставке это обычно от 1 млн ₽. Часто выгоднее взять версию послабее — спросите у специалиста.</p>') +
